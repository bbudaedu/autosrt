...
為了更穩定地處理較長的轉錄文本，`sheets_gemini_processor.py` 內部實現了對提交給 Gemini API 的文本進行分批處理的機制。腳本會將一個文件的完整轉錄內容按照預設的行數上限（當前內部設置為 `GEMINI_API_BATCH_MAX_LINES = 100` 行）分割成若干批次。每個批次會單獨發送給 Gemini API 進行校對，並應用相同的重試邏輯。所有批次成功處理後，結果會被合併。
